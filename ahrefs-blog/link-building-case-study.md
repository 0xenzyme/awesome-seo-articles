...
canonical: "https://ahrefs.com/blog/link-building-case-study/"
author: "Joshua Hardwick"
published: "2020-08-18T16:00:27+00:00"
updated: "2024-10-04T16:39:35+00:00"
categories:
  - "Data & Studies"
  - "Link Building"
freshness_reasons:
...
fetched_at: "2026-06-12T11:45:12+00:00"
status_code: 200
html_hash: "d3e9719dcf6c764fcc98875f85edb7924b0f71a9fbf703ee515591478324e578"
clean_word_count: 3563
clean_char_count: 20748
---
# Link Building Case Study: How We Built Backlinks With a ‘Stats’ Page (And Ranked #1)

A few months ago, we created [a list of SEO statistics](https://ahrefs.com/blog/seo-statistics/).
...
- [Why we chose a statistics page](#why-a-statistics-page)
- [How we created the page and built links to it](#what-we-did)
- [Detailed results from our campaign](#our-results)
- [How we could have improved our results](#improving-results)
...
You’ve probably heard of the [Skyscraper Technique](https://ahrefs.com/blog/skyscraper-technique/). It’s a [link building tactic](https://ahrefs.com/blog/link-building-strategies/) where you find a page with lots of links, create something “better,” then pitch your new and improved resource to those linking to the now inferior page.
...
For example, look at [this post from Shopify](https://www.shopify.com/blog/start-a-youtube-channel-for-your-business) about starting a YouTube channel for your business:
...
If we look at the source of that statistic, it takes us to [YouTube’s Press page](https://www.youtube.com/about/press/). Here, it says there are over two billion monthly YouTube users instead of the cited 1.9 billion.

That’s an issue because it leads bloggers and journalists to cite outdated statistics.

For example, if we check the **Backlinks** report for this page in [Site Explorer](https://ahrefs.com/site-explorer), search for “1.9 billion” in the link anchors and surrounding texts, and toggle the “one link per domain” switch, we see 55 websites citing the outdated statistic and linking to the page.
...
For example, let’s check the **Anchors** report for the list of YouTube statistics in [Site Explorer](https://ahrefs.com/site-explorer). Many referring pages cite a statistic about YouTube being the third most visited site on the web. However, if we search the page itself for that statistic, it’s nowhere to be found.
...
To start, we searched in Ahrefs’ [Keywords Explorer](https://ahrefs.com/keywords-explorer) for keywords related to our business like SEO, search engine optimization, content marketing, etc.
...
This gave us a bunch of ideas, but we needed to narrow them down to those with top-ranking pages that had lots of backlinks. For this, we used the [Keyword Difficulty](https://ahrefs.com/blog/keyword-difficulty/) filter to exclude all low-difficulty keywords.
...
To do this, we opened up the **Anchors** report for the page in [Site Explorer](https://ahrefs.com/site-explorer) and looked for frequently mentioned stats. Right away, we saw a lot of links referencing how 93% of online experiences begin with a search engine.
...
This was easy enough to do. We just added a column for the referring website and used [a tool like this one](https://websiteseochecker.com/extract-url-to-domain/) to extract the root domains in batches. Then we pasted them into our sheet.
...
The final step was to exclude websites without a lot of traffic. To do this, we pulled domain-level traffic for all prospects using the [Ahrefs API](https://ahrefs.com/api) and the script editor in Google Sheets.

If you’re following along and don’t have an API subscription, you can use our [Batch Analysis tool](https://ahrefs.com/batch-analysis) instead. Just paste in up to 200 domains at a time and set the target mode to *domain with all its subdomain*s.
...
Having published our list of SEO statistics (and noindexed the page), we were almost ready to do some outreach and build some links. But first, we needed to [find contact information](https://ahrefs.com/blog/find-email-address/) for our prospects.

If you’ve read our [guide to link building at scale](https://ahrefs.com/blog/fast-link-building/) or watched [our video](https://www.youtube.com/watch?v=HyIXvIaUlLk) on the same topic, then you’ll know that we’re fans of using APIs and automation for this.
...
> **[Pitch]**
>
> We published this and a few other fresh SEO stats here:**https://ahrefs.com/blog/57-seo-statistics-for-2020/**
>
...
Let’s break things down by [Domain Rating](https://ahrefs.com/website-authority-checker):
...
Given that we put a lot of effort into this campaign and only came away with backlinks from 32 websites, you might question whether [blogger outreach](https://ahrefs.com/blog/blogger-outreach/) is still worth the effort. Luckily, the answer to this question is simple: of course it is.

Backlinks are still an important [ranking signal](https://ahrefs.com/blog/google-ranking-factors/), and there’s just no other way to [build high-quality white-hat links](https://ahrefs.com/blog/high-quality-backlinks/) without outreach. Although we executed this campaign over a few weeks, it’s also worth noting that it wasn’t all that complicated to set up. If we were to do it again, assuming that we had someone vetting prospects and finding contact information, we think we could set up the entire campaign in one working day.
